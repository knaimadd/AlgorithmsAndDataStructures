...
class BinaryTree():
    ...
    def add_regular(self, value):
        for j in self.preorder_list():
            if not type(j) in (float, int):
                raise Empty("Can't regularly add a value to a BinaryTree with non-number nodes")
        if not type(value) in (float, int):
            raise Empty("Can't add regularly a non-number value")
        i = 0
        while self._exist(i):
            if value <= self[i]:
                i = i * 2 + 1
